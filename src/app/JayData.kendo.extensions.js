...
(function () {
  if (typeof define === 'function' && define.amd)
    define(["underscore","kendoJayData"], factory);
  else factory(_);
  ...
  function factory(_) {
    ...
    function gridFactory(element, entity, dataSourceOptions, gridOptions, success) {
      ...
      if (mustAutoSave) grid.data("kendoGrid").bind("save", _.debounce(function () {
        var that = this;
      ...
      return { grid: grid, promis: def, requestEnd: requestEnd, requestEndLocal: requestEndLocal };
      /// Locals ///
      var kendoLoader;
    ...
    function makeKendoColumns(entitySet, exclude, onKendoColumnCreated) {
      ...
      entitySet.defaultType.memberDefinitions.getPublicMappedProperties().forEach(function (pd) {
        var col = exclude.filter(function (c) { return c.field == pd.name; })[0] || _.find(columns, function (c) { return c.field == pd.name; });
      ...
      var columnsOut = $.map(columns, function (c, n) {
        var inArray = $.inArray(c.field, _.filter(exclude, function (field) { return _.isString(field); }));
        return include(inArray) ? c : null;
      });
      if (excludeNot)
        columnsOut = $.map(exclude, function (e, n) {
          var column = _.find(columnsOut, function (col) { return col.field == e; });
          return column;
        });
      return columnsOut;
      /// Locals
